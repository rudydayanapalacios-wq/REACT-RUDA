...
class HistorialAcceso(Base):
    ...
    fecha_acceso = Column(DateTime, nullable=True)
...
class Venta(Base):
    ...
    id = Column(Integer, primary_key=True, index=True, autoincrement=True)
    numero_factura = Column(String(40), unique=True, nullable=False)
    usuario_id = Column(Integer, ForeignKey("usuarios.id"), nullable=False)
    fecha = Column(DateTime, server_default=func.getdate(), nullable=False)
